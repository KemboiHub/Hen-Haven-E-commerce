...
const PaymentForm: React.FC<PaymentFormProps> = ({ initialPhone, initialAmount, onSuccess, onClose }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-sage-800">M-Pesa Payment</h2>
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-sage-200 rounded-lg focus:ring-2 focus:ring-sage-500 focus:border-transparent"
              placeholder="2547XXXXXXXX"
              required
            />
          </div>
          <div className="mb-4">
            <label className="block text-sm font-medium text-sage-700 mb-1">Amount (Ksh)</label>
            <input
              type="number"
              value={amount}
          ...
          <button
            type="submit"
            disabled={processing}
            className="w-full bg-sage-600 text-white py-2 rounded hover:bg-sage-700 disabled:opacity-60"
          >
  ...
  return (
    <header className="bg-white shadow-lg sticky top-0 z-50">
      ...
      {showPaymentForm && (
        <PaymentForm
          initialPhone={String((user as any)?.phone || (user as any)?.phoneNumber || (user as any)?.mobile || (user as any)?.contact || '').trim().replace(/^\+/, '').replace(/^0/, '254')}
          initialAmount={Math.round(total)}
